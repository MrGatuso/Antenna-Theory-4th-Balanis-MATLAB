...
ERR = 1;
while(ERR ~= 0)
    fprintf('\nNeed to remove part of the pattern between two thetas? (Y/N)\n');
    fprintf('--------------------------------------------------------------\n');
    ff = input('Input: ','s');
    if (ff == 'Y') | (ff == 'y')
        ERR1=1;
        while(ERR1 ~= 0)
           fprintf('\nRemove the pattern between theta #1 and theta #2\n'); 
           fprintf('\nPlease specify theta #1 and theta #2 in the form "[theta#1,theta#2]"\n');
           fprintf('(in degrees), for example: [0,90]\n');
           fprintf('\n------------------------------------------------------------------------\n');           
           theta_int = str2num(input('Input: ','s'));
           s = size(theta_int);
           if (s(1) ~= 1) | (s(2) ~= 2)
               fprintf('\nPlease specify theta #1 and theta #2 in the form "[theta#1 theta#2]"\n');
           elseif ((theta_int(1) < 0) | (theta_int(1) > theta_int(2)) | (theta_int(2) > 180))
               fprintf('\n   *** ERROR: Theta must satisfy: 0 < theta#1 < theta#2 < 180 (degrees)")!\n\n');
           else
               ERR1 = 0;
           end
        end
...
ERR = 1;
while(ERR ~= 0)
    fprintf('\nNeed to remove part of the pattern between two phis? (Y/N)\n');
    fprintf('------------------------------------------------------------\n');
    ff = input('Input: ','s');
    if (ff == 'Y') | (ff == 'y')
        ERR1=1;
...
if (cut == 1) & (cutt ~= 1)
    ...
    [idx,idy] = find((PHI>phi_int(1))&(PHI<phi_int(2)));
    THETA(idx,idy) = NaN;
    PHI(idx,idy) = NaN;
elseif (cut ~= 1) & (cutt == 1)
    theta_int=(round((theta_int-thrg(1))/del_th)*del_th+thrg(1))*pi/180;
    [idx,idy] = find((THETA>theta_int(1))&(THETA<theta_int(2)));
    THETA(idx,idy) = NaN;
    PHI(idx,idy) = NaN;
elseif (cut == 1) & (cutt == 1)
    phi_int=(round((phi_int-phirg(1))/del_phi)*del_phi+phirg(1))*pi/180;
    theta_int=(round((theta_int-thrg(1))/del_th)*del_th+thrg(1))*pi/180;
    [idx,idy] = find((PHI>phi_int(1))&(PHI<phi_int(2)) ...
        &(THETA>theta_int(1))&(THETA<theta_int(2)));
    THETA(idx,idy) = NaN;
    PHI(idx,idy) = NaN;
end
...
if cutt == 1
    thetac3=ones(size(theta))*theta_int(1);
    [XCT3,YCT3,ZCT3]=CrossSecT(thetac3,phi,ratio,scl,rmin);
    thetac4=ones(size(theta))*theta_int(2);
    [XCT4,YCT4,ZCT4]=CrossSecT(thetac4,phi,ratio,scl,rmin);
end
...
elseif (cut ~= 1) & (cutt == 1)
    cst3=mesh(XCT3,YCT3,ZCT3);
    set(cst3,'linestyle','none');
    cst4=mesh(XCT4,YCT4,ZCT4);
    set(cst4,'linestyle','none');
    Bdyplot(theta_int(1),phi(1),ratio,scl,rmin);
    Bdyplot(theta_int(2),phi(1),ratio,scl,rmin);
    Bdyplot(theta_int(1),phi(length(phi)),ratio,scl,rmin);
    Bdyplot(theta_int(2),phi(length(phi)),ratio,scl,rmin);
elseif (cut == 1) & (cutt == 1)
    csp3=mesh(XCP3,YCP3,ZCP3);
    set(csp3,'linestyle','none');
    csp4=mesh(XCP4,YCP4,ZCP4);
    ...
    cst4=mesh(XCT4,YCT4,ZCT4);
    set(cst4,'linestyle','none');
    Bdyplot(theta_int(1),phi_int(1),ratio,scl,rmin);
    Bdyplot(theta_int(1),phi_int(2),ratio,scl,rmin);
    Bdyplot(theta_int(2),phi_int(1),ratio,scl,rmin);
    Bdyplot(theta_int(2),phi_int(2),ratio,scl,rmin);
end
