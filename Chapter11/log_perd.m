...
%------------------------------------------------------------------------------------------------    
% Length of source transmission line
%------------------------------------------------------------------------------------------------    
            ERR = 1;
    while(ERR ~= 0)
        LLin = str2num(input('Length of source transmission line (in m) = ','s'));
        if (isempty(LLin))
            fprintf('\n   *** ERROR: Please enter a number!\n\n');
        elseif (LLin < 0)
            ...
            ERR = 0;
                ...
                ERR = 1;
    while(ERR ~= 0)
        ZCin = str2num(input('Characteristic impedance of source transmission line (in Ohms) = ','s'));
        if (isempty(ZCin))
            fprintf('\n   *** ERROR: Please enter a number!\n\n');
        elseif (ZCin <= 0)
...
%------------------------------------------------------------------------------------------------    
% Length of termination transmission line
%------------------------------------------------------------------------------------------------    
            ERR = 1;
    while(ERR ~= 0)
        LLout = str2num(input('Length of termination transmission line (in m) = ','s'));
        if (isempty(LLout))
            fprintf('\n   *** ERROR: Please enter a number!\n\n');
        elseif (LLout < 0)
    ...
    ERR=1;
    while(ERR ~= 0)
        Zout = str2num(input('Characteristic impedance of termination transmission line (in Ohms) = ','s'));
        if (isempty(Zout))
            fprintf('\n   *** ERROR: Please enter a number!\n\n');
        elseif (Zout <= 0)
...
if SELECT_mode ==1
    ...
    fprintf('Note: Please enter the diameter from the smallest one to the largest one.\n');
...
function z = cosint(x)
Gamma = .57721566490153286060651209;
F=@(t)(cos(t)-1)./t;
z=Gamma+log(x)+quad(F, 0, x );     
end
